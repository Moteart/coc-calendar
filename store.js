...
window.Store = (function () {
  ...
  function normalize(raw) {
    ...
    out.sessions.forEach(function (x) { if (x.duration === undefined) x.duration = null; });
    return out;
  }
  ...
  function makeInst(s, mod, date) {
    ...
    var status = (ov && ov.status) || 'planned';
  ...
  function expand(sd, ed) {
    ...
    sessions().forEach(function (s) {
      ...
      if (end < cur) return;
      var diff = (s.weekday - weekdayOf(cur) + 7) % 7;
  ...
  function addSession(data) {
    var s = {
      ...
      type: data.type === 'weekly' ? 'weekly' : 'once',
      weekday: data.weekday != null ? +data.weekday : weekdayOf(data.date || todayStr()),
      time: data.time || '20:00',
      date: data.type === 'weekly' ? null : (data.date || todayStr()),
      startDate: data.type === 'weekly' ? (data.startDate || todayStr()) : null,
      endDate: data.type === 'weekly' ? (data.endDate || null) : null,
  ...
  return {
    ...
    removeInst: removeInst, restoreInst: restoreInst,
    // 设置
