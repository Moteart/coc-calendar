...
self.addEventListener('install', e => {
  e.waitUntil(
    caches.open(CACHE_NAME)
      .then(c => Promise.allSettled(NET_FIRST.concat(CACHE_FIRST).map(a => c.add(a))))
      .then(results => {
        // 预缓存失败时输出警告（部署路径错误时便于排查）
        if (self.console && results) {
          results.forEach(function (r, i) {
            if (r && r.status === 'rejected') console.warn('[SW] precache fail:', NET_FIRST.concat(CACHE_FIRST)[i]);
          });
        }
      })
...
self.addEventListener('fetch', e => {
  ...
  const isNetFirst = NET_FIRST.some(p => pathname === p || pathname === p.replace(/^\//, ''));
  if (isNetFirst) {
    e.respondWith(
  ...
  const isCacheFirst = CACHE_FIRST.some(p => pathname === p);
  if (isCacheFirst) {
    e.respondWith(
